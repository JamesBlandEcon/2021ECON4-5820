...
d<-list(
  N = dim(Part1)[1],
  n = length(unique(Part1$Subject)),
  invest = UseInvest,
  chi = Part1$chi,
  Part=1+1*(Part1$Part=="Part 1"),
  UseData=as.vector(matrix(1,dim(Part1)[1],1)),
  ID=Part1$Subject,
  priorR=c(log(0.3),0.2,log(0.1),0.2),
  priorL=c(log(0.01),0.2,log(0.1),0.2)
) 
  
Fit<-stan("Bracketing.stan",data=d,control = list(adapt_delta = 0.999,max_treedepth=15))
